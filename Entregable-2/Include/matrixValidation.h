...
bool regexFloat(string numb){ //Funcion que regresa un booleano referente a si la entrada recibida es o no un entero.
...
vector<vector<float>> clearMatrix (vector<vector<float>> matrix) {
  matrix.clear();
  return matrix;
}

vector<vector<float>> generalValidations (vector<vector<string>> adjMatrix, int matrixNumber) {
  vector<vector<float>> validatedMatrix (adjMatrix.size(), vector<float> (adjMatrix.size(), 0));

  if (adjMatrix.size() != adjMatrix[0].size()) return clearMatrix(validatedMatrix);

  for (int index = 0; index < adjMatrix.size(); index++) {
    if (adjMatrix[index][index] != "0") return clearMatrix(validatedMatrix); // Check same node connections 0,0 1,1 ...

    for (int number = 0; number < adjMatrix.size(); number++) { // Check that text entered is float
      if (!regexFloat(adjMatrix[index][number])) return clearMatrix(validatedMatrix);

      float numberToCheck = stof(adjMatrix[index][number]);
      // Check that matrix 1 only has -1 as negative numbers
      if (index != number) if (matrixNumber == 1 && (numberToCheck < -1 || numberToCheck == 0)) return clearMatrix(validatedMatrix);
...
void adjustFlowMatrix (int index, int element, vector<vector<float>> &matrix2) {
...
bool matrixCorrespondence (vector<vector<float>> &matrix1, vector<vector<float>> &matrix2) {
    for (int index = 0; index < matrix1.size(); index++) {
      for (int element = 0; element < matrix1.size(); element++) {
        ...
        if (elementMatrix_1 == -1 && elementMatrix_2 != 0) return false;
        if (index != element) if (elementMatrix_2 == 0 && elementMatrix_1 != -1) return false;

        if (elementMatrix_2 < 0) adjustFlowMatrix(index, element, matrix2);
      }
    }
    return true;
}
