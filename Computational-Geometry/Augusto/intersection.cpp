...
bool todoEnUno(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4){ //O(1)
    float a = ((y2 -y1) / (x2-x1));
...
bool regexNumber(string numb){ //Funcion que regresa un booleano referente a si la entrada recibida es o no un entero.
...
void initializer(){
    ...
    while(true){ //Mientras el numero de elementos introducido no sea correcto.
        ...
        if(!regexNumber(N)){ //Si se introduce un caracter erroneo.
            cout<<"ERROR: Solo se aceptan caracteres numericos del 0 al 9, sin espacios.\n";
        }else if(stoi(N) < 2){//Si se introduce un valor menor a 3 elementos.
    ...
    for(int g = 0; g < n; g++){
        for(int h = 0; h < 4; h++){
            ...
            while(true){ //Mientras no se hayan agreagdo todos los pesos.
            ...
            if(!regexFloat(currentN)){ //Si la entrada no es valida.
